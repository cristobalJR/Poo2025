 ...
 * tambien en un mapa por nombre para buscarlas rápido.
 */
// EXTENSION ->
/**
...
public class TrainNetwork {
    ...
    private final TreeSet<TrainStation> stationSet = new TreeSet<>();        // EXTENSION -> únicas y ordenadas
    private final HashMap<String, TrainStation> stationMap = new HashMap<>(); // EXTENSION -> búsqueda por nombre
    private final Set<String> stationsUnderWorks = new HashSet<>();          // EXTENSION -> nombres de estaciones en obras
    private final TariffCalculator calculator;

    public TrainNetwork() {
        loadStationGraph(STATIONS_FILE);
        loadStationsUnderWorks(WORKS_FILE); // EXTENSION -> estaciones que están en obras
        // EXTENSION -> La lista de zonas (ordenada) se construye a partir de las estaciones
    ...
    private void loadStationGraph(String fileName) {
        // EXTENSION -> Lectura con el patrón clásico try/catch/finally: el recurso se declara
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                // EXTENSION -> Quitamos comillas y separamos por comas: línea, nombre, zona.
            ...
            // EXTENSION -> El cierre también puede lanzar IOException, así que se protege.
            if (reader != null) {
                try {
                    reader.close();
                } catch (IOException e) {
    ...
    /** Devuelve las zonas distintas de estaciones, ordenadas. */
    // EXTENSION ->
