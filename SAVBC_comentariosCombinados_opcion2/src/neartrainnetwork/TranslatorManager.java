...
public class TranslatorManager {
    ...
    private Translator currentTranslator;                        // EXTENSION -> idioma activo
    private final Map<String, Translator> translatorMap = new LinkedHashMap<>();

    public TranslatorManager() {
        loadDefaultTranslator(); // EXTENSION -> primero el español (por defecto)
        loadTranslators();       // EXTENSION -> luego los idiomas de la carpeta dictionaries/
    }
    ...
    private void loadTranslators() {
        ...
        if (files == null) {
            return; // EXTENSION -> la carpeta aún no existe: solo habrá español
        }
        for (File file : files) {
            String name = file.getName();
            if (!name.toLowerCase().endsWith(".txt")) {
                continue; // EXTENSION -> ignoramos lo que no sea .txt
            }
            // EXTENSION -> El nombre del idioma es el del fichero sin la extensión.
